...
import com.tower.common.bean.ReportTower;
import com.tower.common.bean.ReportTowerPage;
import com.tower.common.bean.ReportTowerParameter;
...
public class ReportWeb {
	...
	public ModelAndView getReportTowers(HttpServletRequest request,
			ReportTowerPage reporttowePrage) {
		if (reporttowePrage == null)
			reporttowePrage = new ReportTowerPage();

		ReportTowerParameter parameter=reporttowePrage.getParameter();
		...
		if (admintype == 2) {
			adminAreas = sysUserInfo.getAreas();
			reporttowePrage.setAdminareas(adminAreas);
		}

		List<City> citys = sysUserInfo.getCitys();
		reporttowePrage.setAdminCitys(citys);
		List<Area> areas = new ArrayList<Area>();// sysUserInfo.getAreas();
		if (parameter.getTowercity() != 0) {
			Map map = new HashMap();
			map.put("cityid", parameter.getTowercity());
		...
		reporttowePrage.setAdminorderstates(ParamerUtil.getAdminReportOrder(sysUserInfo.getAdminpower()));
		List<ReportTower> towers = reportService.getReportTowers(reporttowePrage);
		reporttowePrage.setTowers(towers);
		reporttowePrage.setTowercitys(citys);
		reporttowePrage.setTowerareas(areas);
		...
		List<OrderState> orderStates = orderService.getOrderStates(map);
		//List<OrderState> orderstates=orderService.getOrderStates(null);
		reporttowePrage.setOrderstates(orderStates);
		*/
		parameter.setToweraddtimeend(todate);
		
		// 设置页面
		PageUtil pageUtil = new PageUtil(reporttowePrage.getPageno(),
				reporttowePrage.getPagerowtotal());
		reporttowePrage.setPageinfo(pageUtil.getPage());
		return new ModelAndView("/report/tower_report", "reportTowerPage", reporttowePrage);
	}
	...
	public void dcTowers(HttpServletRequest request,
			HttpServletResponse response, ReportTowerPage reporttowePrage)  throws Exception {
		...
		reporttowePrage.setAdminCitys(citys);
		List<Area> areas = new ArrayList<Area>();// sysUserInfo.getAreas();
		if (reporttowePrage.getParameter().getTowercity() != 0) {
			Map map = new HashMap();
			map.put("cityid", reporttowePrage.getParameter().getTowercity());
			if (admintype == 2) {
				map.put("areaids", sysUserInfo.getAdminarea().split(","));
			}
		...
		try {
			if (reporttowePrage.getParameter().getTowername() != null) {
				reporttowePrage.getParameter().setTowername(new String(reporttowePrage.getParameter().getTowername() .getBytes("ISO8859-1"), "utf-8"));
			}
		} catch (UnsupportedEncodingException e) {
		}
		
		try {
			if (reporttowePrage.getParameter().getUsername() != null) {
				reporttowePrage.getParameter().setUsername(new String(reporttowePrage.getParameter().getUsername() .getBytes("ISO8859-1"), "utf-8"));
			}
		} catch (UnsupportedEncodingException e) {
		}
		String filename="站址报表("+TimeUtil.getNowDay()+").xls";
		ExcelDC ecDc = new ExcelDC(request, "report_tower.xls");
		reporttowePrage.setAdminorderstates(ParamerUtil.getAdminReportOrder(sysUserInfo.getAdminpower()));
		List<Map> list = reportService.getDCReportTowers(reporttowePrage);
		
		HSSFWorkbook workbook = ecDc.getReportTowers(list);
		 ByteArrayOutputStream os = new ByteArrayOutputStream();
